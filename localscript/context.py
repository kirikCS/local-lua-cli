...
class Context:
    ...
    def memory_enable(self) -> dict:
        ...
        for msg in self.iterations:
            ...
            # The agent uses role="user" for tool results too. Tag them
            # explicitly so retrieval can distinguish if it ever wants to.
        ...
        try:
            ...
            pass
        # Phase 2: embed any pre-existing rows that don't yet have a vector.
        # This is the "first run after upgrade" backfill — also kicks in when
        # the user pulls nomic-embed-text after using BM25-only for a while.
    ...
    def compact(self) -> dict:
        """Compact context: summarize first 70%, keep last 30%.

        The summary is rendered as a structured markdown block with four
        sections: Tasks Requested, Files (current state on disk),
        Errors & Resolutions, and Current Project State. Tasks and files
        are extracted deterministically; errors and state are synthesized
        by the LLM with a strict JSON schema.

        Returns {"before": int, "after": int, "summarized": int, "kept": int}.
        """
        n = len(self.iterations)
        if n <= 4:
            return {"before": n, "after": n, "summarized": 0, "kept": n}

        # Split: first 70% gets summarized, last 30% stays
