...
def _handle_slash(cmd: str, ctx: Context) -> Context:
    ...
    if base == "/help":
        ...
        if result["summarized"] == 0:
            ui.console.print("  [dim]Nothing to compact[/dim]")
        else:
            ui.console.print(
        ...
        if sub == "--on" or sub == "on":
            try:
                ...
                ui.console.print(f"  [red]Memory enable failed: {e}[/red]")
        ...
        if tracker:
            ui.console.print(tracker)
        else:
            ui.console.print("  [dim]No active task plan[/dim]")
    elif base == "/clear":
        ctx.clear()
...
def _repl():
    ...
    try:
        while True:
            ...
            ui.console.print()
            def _confirm_plan(subtasks):
                ui.console.print("  [bold]Task Plan:[/bold]")
                for j, s in enumerate(subtasks, 1):
                    ui.console.print(f"    {j}. {s['title']} \u2014 {s['description']}")
                try:
                    r = ui.console.input("  [dim]Execute? [Y/n][/dim] ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    return False
                return r not in ("n", "no")

            try:
                agent.run(user_input, ctx=ctx, on_plan=_confirm_plan)
                ui.console.print()
