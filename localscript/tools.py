    ...
    try:
        ...
        for line in result.stdout.strip().split("\n"):
            if ":" in line and ("W" in line or "E" in line):
                ...
                clean = clean.replace(tmp.replace("\\", "/"), display_name)
                errors.append(clean)
...
def lua_execute(code: str, timeout: int = EXECUTE_TIMEOUT, display_name: str = "file.lua") -> dict:
    """Run Lua code via lua54. Returns {success, stdout, stderr}."""
    with tempfile.NamedTemporaryFile(
        suffix=".lua", mode="w", delete=False, encoding="utf-8"
    ) as f:
    ...
    try:
        ...
        stderr = result.stderr.replace(tmp, display_name).replace(tmp.replace("\\", "/"), display_name)
