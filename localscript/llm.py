...
# ---------------------------------------------------------------------------
# Model capability cache — which models accept `think` at all
# ---------------------------------------------------------------------------

# Maps model tag -> bool (True = supports thinking, False = non-thinking).
# Populated on first send; on cache miss we try sending think and fall back
# if Ollama returns the 400 "does not support thinking" error.
_THINKING_SUPPORT: dict[str, bool] = {}


def _supports_thinking(model: str) -> bool | None:
    """Cached check. Returns None if unknown (try it, learn from the response)."""
    return _THINKING_SUPPORT.get(model)


def _mark_thinking_support(model: str, supported: bool) -> None:
...
def generate(messages: list,
    ...
    If the model does not support thinking (e.g. fine-tuned Qwen3-Instruct),
    the `think` parameter is silently skipped — Ollama 400s otherwise.
    """
    ...
    # Ollama native: 'think' at top level. Only send it for models that
    # actually support thinking — otherwise Ollama returns a 400.
    if enable_thinking is not None and _supports_thinking(model) is not False:
    ...
    try:
        ...
        # First-time discovery: a model declared without thinking capability
        # will 400 on `think: ...`. Cache the result and retry without it.
        if (e.response is not None
                and e.response.status_code == 400
                and "does not support thinking" in (e.response.text or "")
                and "think" in body):
            _mark_thinking_support(model, False)
            body.pop("think", None)
            resp = requests.post(config.LLM_URL, json=body, stream=True, timeout=180)
            resp.raise_for_status()
        else:
        ...
        # Successful send — if we sent `think`, this model supports it.
        if "think" in body and _supports_thinking(model) is None:
            _mark_thinking_support(model, True)
    resp.encoding = "utf-8"
