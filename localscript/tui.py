...
class IceqApp(App):
    ...
    def _run_compact(self):
        ...
        try:
            ...
            self._post(AgentBlock(
                run_id, f"[red]Compact failed: {escape(str(e))}[/red]",
            ))
    ...
    def _run_memory_enable(self):
        ...
        memory_enable() can be slow when phase 2 is active: it embeds the
        whole current iterations buffer AND backfills embeddings for any
        pre-existing rows from prior sessions, both via Ollama. Running it
        on the UI thread freezes input until the embed calls return. This
        worker keeps the TUI responsive while the embeds happen.
        """
        run_id = self._agent_run_id
        try:
            ...
            self._post(AgentBlock(
                run_id, f"[red]Enable failed: {escape(str(e))}[/red]",
            ))
